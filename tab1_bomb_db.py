...
class Tab1(Tk):
    ...
    def dbclass_start(self): 
        ...
        BombCrypto(self.bcoin_total.get(), self.heroes.get())
...
class BombCrypto:
    ...
    def bcoin_today(self):
        if self.day > 1:
            last_day = self.day - 1
            self.cur.execute(f'SELECT bcoin_total FROM bombcrypto WHERE day = {last_day}')
    ...
    def bcoin_media(self): 
        ...
        if self.day > 1:
            average = []
            average.append(self.bcoin_earning)
            for i in avg[0]:
                average.append(i)
                
            result = (sum(average)) / len(average)
            self.bcoin_avg = result
